...
_IDLE_SCV = actions.FUNCTIONS.select_idle_worker.id
...
#smart_actions.append(ACTION_INACTIF_SCV)
...
class SparseAgent(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        if self.move_number == 0:
            ...
            excluded_actions = []
            if (supply_used - supply_limit) > 6 or worker_supply == 0:
                excluded_actions.append(1)
                
            if supply_depot_count == 0 or worker_supply == 0:
            ...
            if smart_action == ACTION_INACTIF_SCV:
				#selectSCV see idle Worker dans l aide
				return actions.FunctionCall(_IDLE_SCV,_SELECT_WORKERS)
			
            elif smart_action == ACTION_BUILD_SCV:
				cc_y, cc_x = (unit_type == _TERRAN_COMMANDCENTER).nonzero()
				if cc_y.any():
					i = random.randint(0, len(cc_y) - 1)
					target = [cc_x[i], cc_y[i]]
